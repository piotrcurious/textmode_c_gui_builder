...
class Color(Enum):
    ...
    @classmethod
    def from_str(cls, s): try: return cls[s.upper()] 
...
class UIElement:
    ...
    def to_dict(self): 
        d = asdict(self); d['color'] = self.color.name; return d
    
    @staticmethod
    def from_dict(d: dict):
        t = d.pop('type'); col = Color.from_str(d.pop('color'))
        if t == "BOX": return Box(color=col, **d)
        if t == "TEXT": return Text(color=col, **d)
        if t == "LINE": return Line(color=col, **d)
        if t == "FREEHAND": return Freehand(color=col, **d)
        return None
...
class GuiManager:
    ...
    def _emit(self, cmd):
        ...
        if data or cmd in ["SAVE_ASSET"]: # Save asset might send None data if cancelled, handled in main
    ...
    def _load_lib(self):
        try: return json.loads(Path("library.json").read_text())
        except: return {}
...
class ProjectManager:
    ...
    def save_project(self, screens: List[Screen]):
        ...
        # --- 1. HEADER GENERATION ---
        # Defines Structs for layout anchors
        h_lines = [
        ...
        # Global Resources (Freehand) - Deduplicated
        processed_res = set()
    ...
    def load_project(self) -> List[Screen]:
        # A robust Regex parser is needed to reverse-engineer the Struct definitions.
        # For simplicity in this demo, we will use a "Save/Load" JSON sidecar for the project data.
        # While "Round-Tripping" from C++ is noble, keeping a .json source of truth 
        # is infinitely more reliable for maintaining object properties like "Name".
        ...
        if Path("project.json").exists():
            try:
                ...
                for s_data in data:
                    ...
                    s.objects = [UIElement.from_dict(o) for o in s_data['objects']]
                    screens.append(s)
                return screens
            except: pass
        return [Screen("Main")]
    ...
    def save_json_state(self, screens):
        ...
        Path("project.json").write_text(json.dumps(data, indent=2))
...
class Designer:
    ...
    @property
    def cur_screen(self): return self.screens[self.act_idx]
    @property
    def cur_objs(self): return self.cur_screen.objects

    def run(self):
        curses.start_color(); curses.use_default_colors(); curses.curs_set(1)
        for i, c in enumerate(Color, 1): curses.init_pair(i, getattr(curses, f"COLOR_{c.name}"), -1)
            ...
            while True:
                ...
                if cmd == "SWITCH_SCREEN":
                    for i, s in enumerate(self.screens): 
                        if s.name == data: self.act_idx = i; self.sel_idx = -1
                elif cmd == "ADD_SCREEN":
                    if data: self.screens.append(Screen(data)); self.act_idx = len(self.screens)-1
                elif cmd == "RENAME_SCREEN":
                    if data:
                        for s in self.screens: 
                            if s.name == data[0]: s.name = data[1]
                elif cmd == "SAVE_ASSET":
                    if self.sel_idx >= 0 and data:
                        obj = self.cur_objs[self.sel_idx]
                        lib = self.gui._load_lib()
                        # Save generic template
                        d = obj.to_dict(); d['name'] = data # Use the asset name
                        lib[data] = d
                        Path("library.json").write_text(json.dumps(lib))
                    ...
                    if data in lib:
                        ...
                        if hasattr(new_obj, 'x'): new_obj.x = self.cx; new_obj.y = self.cy
                        if hasattr(new_obj, 'x1'): 
                            w = abs(new_obj.x2 - new_obj.x1)
                            h = abs(new_obj.y2 - new_obj.y1)
                            new_obj.x1 = self.cx; new_obj.y1 = self.cy
                            new_obj.x2 = self.cx + w; new_obj.y2 = self.cy + h
                        # Ensure unique name in current screen
                        base = new_obj.name
                        cnt = 1
                        while any(o.name == new_obj.name for o in self.cur_objs):
    ...
    def _handle_key(self, k):
        if k == 27: self.mode = Mode.NAV; self.sel_idx = -1; return True
        ...
        if (dx or dy):
            if self.mode == Mode.NAV and self.sel_idx >= 0:
        ...
        if self.mode == Mode.NAV:
            ...
            if k == ord('s'): 
                ...
                self.msg = "Project Saved!"; return True
            if k == 9: # Tab
            ...
            if k == ord('d') and self.sel_idx >= 0: self.cur_objs.pop(self.sel_idx); self.sel_idx = -1
            if k == ord('c') and self.sel_idx >= 0: 
                o = self.cur_objs[self.sel_idx]
                colors = list(Color)
                idx = colors.index(o.color)
                o.color = colors[(idx+1)%len(colors)]
            if k == ord('n') and self.sel_idx >= 0:
                self._rename_obj()
            ...
            self.cur_objs.append(Box(name, Color.WHITE, x, y, w, h))
            ...
            self.cur_objs.append(Line(name, Color.WHITE, self.temp['x'], self.temp['y'], self.cx, self.cy))
            self.mode = Mode.NAV

        return True

    def _add_text(self):
        curses.echo(); self.stdscr.addstr(0,0,"Text: "); t = self.stdscr.getstr().decode(); curses.noecho()
        if t:
            name = f"txt_{len(self.cur_objs)}"
            self.cur_objs.append(Text(name, Color.WHITE, self.cx, self.cy, t))

    def _rename_obj(self):
        # We need to pause curses to show a popup? 
        # Actually simpler to use Tkinter prompt but thread sync is messy. 
        # Use curses simple prompt.
        curses.echo(); self.stdscr.addstr(0,0,"New Name: "); t = self.stdscr.getstr().decode(); curses.noecho()
        if t and self.sel_idx >= 0:
            self.cur_objs[self.sel_idx].name = re.sub(r'[^a-zA-Z0-9_]', '', t)
    ...
    def _draw_interface(self):
        ...
        for i, o in enumerate(self.cur_objs):
            ...
            try:
                if isinstance(o, Box):
                    self.stdscr.addch(o.y, o.x, '+', attr); self.stdscr.addch(o.y+o.h-1, o.x+o.w-1, '+', attr)
                    for k in range(1, o.w-1): self.stdscr.addch(o.y, o.x+k, '-', attr); self.stdscr.addch(o.y+o.h-1, o.x+k, '-', attr)
                    for k in range(1, o.h-1): self.stdscr.addch(o.y+k, o.x, '|', attr); self.stdscr.addch(o.y+k, o.x+o.w-1, '|', attr)
                elif isinstance(o, Text): self.stdscr.addstr(o.y, o.x, o.content, attr)
                elif isinstance(o, Line): 
                    # Simple Line Preview
                    self.stdscr.addline(o.y1, o.x1, o.y2, o.x2, attr)
                elif isinstance(o, Freehand):
                    for r, l in enumerate(o.lines): self.stdscr.addstr(o.y+r, o.x, l, attr)
            except: pass

        # Status & Cursor
        h, w = self.stdscr.getmaxyx()
        sel_name = f"Sel: {self.cur_objs[self.sel_idx].name}" if self.sel_idx >= 0 else ""
        stat = f"[{self.cur_screen.name}] {self.msg} | {sel_name} | Pos: {self.cx},{self.cy}"
        self.stdscr.addstr(h-1, 0, stat[:w-1], curses.A_REVERSE)
        try: self.stdscr.addch(self.cy, self.cx, 'X', curses.A_REVERSE)
        except: pass
        self.stdscr.refresh()
        ...
        help_txt = (
            ...
            "S: Save Project\n\n"
            "HINT:\n"
            "Use 'Save Selected to Library' button\n"
            "to save reusable UI components."
        )
