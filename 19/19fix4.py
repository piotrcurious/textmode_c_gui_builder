...
# ==============================================================================
# 3) GUI & ASSET MANAGER (FIXED: focus-aware, non-destructive updates)
# ==============================================================================
...
class GuiManager:
    ...
    def _run(self):
        ...
        lbl_help = tk.Label(f_help, textvariable=self.help_var, justify="left", 
                            font=("Consolas", 10), wraplength=400, bg="#f0f0f0", relief="groove")
        lbl_help.pack(fill="both", expand=True, padx=5, pady=5)

        # -- TAB 2: SCREENS --
        f_scr = tk.Frame(tabs)
        tabs.add(f_scr, text="Screens")
        self.lst_scr = tk.Listbox(f_scr, height=6, exportselection=False)
        self.lst_scr.pack(fill="x", padx=5, pady=5)
        
        btn_fr = tk.Frame(f_scr)
        btn_fr.pack(fill="x", pady=5)
        tk.Button(btn_fr, text="Switch To", command=lambda: self._emit("SWITCH_SCREEN")).pack(side="left", padx=5)
        tk.Button(btn_fr, text="New Screen", command=lambda: self._emit("ADD_SCREEN")).pack(side="left")
        tk.Button(btn_fr, text="Rename", command=lambda: self._emit("RENAME_SCREEN")).pack(side="left", padx=5)
        ...
        a_btn_fr.pack(fill="x", pady=5)
        tk.Button(a_btn_fr, text="Insert Selected", bg="#ddffdd", command=lambda: self._emit("INSERT_ASSET")).pack(side="left", padx=5)
        tk.Button(a_btn_fr, text="Delete Asset", command=self._delete_asset).pack(side="right", padx=5)
        ...
        # Bindings to help preserve selection state and to mark user-interaction
        # We track focus to avoid stomping user selection during updates.
        def _on_focus_in(event):
            # no-op placeholder, having the binding prevents some weird focus behavior
            pass
        self.lst_scr.bind("<FocusIn>", _on_focus_in)
        self.lst_assets.bind("<FocusIn>", _on_focus_in)

        self.ready.set()
        self.root.mainloop()

    # helper: safely get all items from a listbox
    ...
    def _emit(self, cmd):
        ...
        # Only push command if user actually confirmed or selected an item
    ...
    def _refresh_assets(self):
        # Preserve selection by name when possible; avoid destructive re-population
        try:
            prev_sel = None
            sel = self.lst_assets.curselection()
            if sel:
    ...
    def update_state(self, help_text, screens, active_scr):
        # Thread-safe small updates using root.after
        if not self.ready.is_set(): 
            return
        try:
            # Always update help text
            self.root.after(0, lambda: self.help_var.set(help_text))

            # Schedule a focused, careful screens update
            def _update_screens():
                try:
                    # If the user is interacting with the screens list (it has focus), don't override selection.
                    lb = self.lst_scr
                    focused = (self.root.focus_get() is lb)
                    incoming = list(screens)
                    cur_items = self._listbox_items(lb)

                    # If list differs, replace it (safe operation)
                    if cur_items != incoming:
                        lb.delete(0, tk.END)
                        for name in incoming:
                            lb.insert(tk.END, name)

                    # If user is focused on the list, preserve their selection; else try to select active_scr
                    if not focused and incoming:
                        try:
                            idx = incoming.index(active_scr) if active_scr in incoming else 0
            ...
            # Schedule assets refresh similarly but non-destructive
            def _update_assets():
                try:
                    # Only refresh if changed; this function delegates to _refresh_assets which preserves selection
                    self._refresh_assets()
                except Exception:
                    pass

            # Post updates to the Tk event loop
            self.root.after(0, _update_screens)
            self.root.after(0, _update_assets)
...
class ProjectManager:
    ...
    def load_project(self) -> List[Screen]:
        if Path("project.json").exists():
            try:
                text = Path("project.json").read_text(encoding="utf-8")
...
class Designer:
    ...
    def _handle_key(self, k):
        ...
        if (dx or dy):
            if self.mode == Mode.NAV and self._valid_sel():
                ...
                # leave room for status bar (one line)
        ...
        if self.mode == Mode.NAV:
            ...
            x = min(self.temp['x'], self.cx)
            y = min(self.temp['y'], self.cy)
            w = abs(self.temp['x']-self.cx)+1
            h = abs(self.temp['y']-self.cy)+1
    ...
    def _add_text(self):
        ...
        if t:
            ...
            new_txt = Text(name=name, color=Color.WHITE, x=self.cx, y=self.cy, content=t, layer=len(self.cur_objs))
    ...
    def _draw_interface(self):
        ...
        # Draw Objects in list order (index 0 drawn first => bottom)
        ...
        # Update Tkinter Text (help + lists)
        help_txt = (
        ...
        try:
            # Provide the active screen name for GUI to optionally select
            names = [s.name for s in self.screens]
            self.gui.update_state(help_txt, names, self.cur_screen.name)
        except Exception:
            pass
...
if __name__ == "__main__":
    try:
        ...
        # Attempt a robust terminal restore
        try:
            curses.nocbreak()
        except Exception:
            pass
