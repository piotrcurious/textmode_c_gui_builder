...
class UIElement:
    ...
    def from_dict(d: dict):
        t = d.pop('type', 'BASE')
        col_str = d.pop('color', 'WHITE')
        col = Color.from_str(col_str)
        
        # We must filter out keys that don't belong to specific subclasses
        # to avoid TypeError on __init__
        common_keys = {'name': d.get('name', 'unk')}
        
        if t == "BOX":
            return Box(name=d['name'], color=col, x=d.get('x',0), y=d.get('y',0), w=d.get('w',0), h=d.get('h',0))
        if t == "TEXT":
            return Text(name=d['name'], color=col, x=d.get('x',0), y=d.get('y',0), content=d.get('content',''))
        if t == "LINE":
            return Line(name=d['name'], color=col, x1=d.get('x1',0), y1=d.get('y1',0), x2=d.get('x2',0), y2=d.get('y2',0))
        if t == "FREEHAND":
            return Freehand(name=d['name'], color=col, x=d.get('x',0), y=d.get('y',0), lines=d.get('lines',[]))
        return None
...
class Designer:
    ...
    @property
    def cur_screen(self): 
        if self.act_idx < len(self.screens):
    ...
    def run(self):
        ...
        while True:
            ...
            # Auto-save state to json on every significant action?
            # Better to do it on manual save or exit to avoid IO lag
    ...
    def _draw_interface(self):
        ...
        try:
            ...
            self.stdscr.move(self.cy, self.cx) # Move physical cursor
        except: pass
