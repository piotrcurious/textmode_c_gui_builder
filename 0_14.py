...
void draw(const Box& box) {
  ...
  if (h_line) {
    ...
    h_line[0] = '+';
    h_line[box.w - 1] = '+';
    h_line[box.w] = '\\0'; // Null-terminate the string
...
class DrawableObject:
  ...
  def draw_on_canvas(self, stdscr, is_selected):
    ...
    try:
      ...
      pass # Ignore drawing off-screen
...
class Designer:
  def __init__(self, stdscr):
    ...
    self.screens = {}
    self.current_screen_name = "default_screen"
    self.objects = []
  ...
  def _handle_input(self, key):
    ...
    if key in [10, 13, curses.KEY_ENTER]:
      ...
      if self.mode in [DesignMode.DRAW_BOX_START, DesignMode.DRAW_BOX_END, DesignMode.DRAW_LINE_START, DesignMode.DRAW_LINE_END]:
    ...
    elif self.mode in [DesignMode.DRAW_BOX_START, DesignMode.DRAW_LINE_START, DesignMode.DRAW_BOX_END, DesignMode.DRAW_LINE_END]:
      self._handle_navigation(key)
  ...
  def _prompt(self, prompt_text):
    ...
    try:
      box.edit(validator)
    except curses.error: # Handles resize during prompt
      pass
  ...
  def _start_drawing(self, shape):
    ...
    self.selected_object_index = -1 # Deselect object before drawing
  ...
  def _finish_drawing(self):
    ...
    # Before adding the new object, deselect any previously selected one
    self.selected_object_index = -1 
    
    # Corrected logic to determine the object type
    obj_type = ""
    if self.mode == DesignMode.DRAW_BOX_END:
        obj_type = "box"
        x_min, y_min = min(x1, x2), min(y1, y2)
        w, h = abs(x1 - x2) + 1, abs(y1 - y2) + 1
        data = (x_min, y_min, w, h)
    ...
    name = self._get_object_name(obj_type)
    if not name:
        self._cancel_drawing()
        return

    self.objects.append(DrawableObject(obj_type, name, data))
  ...
  def _get_object_name(self, obj_type):
    name = self._prompt(f"Enter C++ variable name for new {obj_type} (leave blank for auto): ")
    if not name:
      base = obj_type.lower()
  ...
  def _new_screen(self):
    ...
    if name:
      if name in self.screens:
        self.objects = self.screens[name]
        self.status = f"Screen '{name}' already exists. Loading it."
      else:
        self.objects = []
        self.screens[name] = self.objects
        self.status = f"New screen '{name}' created."
      self.current_screen_name = name
      self.selected_object_index = -1
    else:
      self.status = "New screen creation cancelled."

  def _switch_screen(self):
    self.mode = DesignMode.SWITCH_SCREEN
    if len(self.screens) <= 1:
      self.status = "No other screens exist to switch to. Create one with 'n'."
      self.mode = DesignMode.NAVIGATE
      return
  
    self.stdscr.clear()
    self.stdscr.addstr(0, 0, "Select a screen to edit:", curses.A_REVERSE)
    screen_names = list(self.screens.keys())
    for i, name in enumerate(screen_names):
      display_name = f"{i+1}. {name}"
      attr = curses.A_BOLD if name == self.current_screen_name else curses.A_NORMAL
      self.stdscr.addstr(i + 2, 2, display_name, attr)
    self.stdscr.refresh()
  
    key = self.stdscr.getch()
    if '1' <= chr(key) <= str(len(screen_names)):
      idx = int(chr(key)) - 1
      # Save the current screen before switching
      self.screens[self.current_screen_name] = self.objects
      
      # Switch to the new screen
      self.current_screen_name = screen_names[idx]
      self.objects = self.screens[self.current_screen_name]
      self.selected_object_index = -1
      self.status = f"Switched to screen '{self.current_screen_name}'."
    else:
      self.status = "Screen switch cancelled."
  ...
  def _draw_ui(self):
    ...
    if self.mode == DesignMode.DRAW_BOX_START:
      ...
      y1, x1 = self.temp_data['y1'], self.temp_data['x1']
      ...
      self.stdscr.addch(self.temp_data['y1'], self.temp_data['x1'], 'X', curses.A_BOLD | curses.color_pair(2))
  ...
  def _save_and_compile(self):
    ...
    try:
      ...
      self.status = "✅ COMPILED successfully! 'ui_generated.ino' is ready. Press 'q' to exit."
...
def compile_to_cpp(screens, filename="ui_generated.ino"):
  ...
  for name in screen_names:
    ...
    # Separate declarations and draw calls
    declarations = []
    draw_calls = []
    objects = screens[name]
    for obj in objects:
      declarations.append(obj.to_cpp_declaration())
      draw_calls.append(obj.to_cpp_draw_call())
