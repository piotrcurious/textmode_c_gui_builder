...
class UIElement:
    ...
    def from_dict(d: Dict[str, Any]) -> Optional["UIElement"]:
        ...
        if t == "META":
            children = [UIElement.from_dict(c) for c in d.get('children', []) if UIElement.from_dict(c) is not None]
            return MetaObject(
                name=str(d.get('name', 'meta')),
                color=color,
                x=int(d.get('x', 0)),
                y=int(d.get('y', 0)),
                children=children,
...
class MetaObject(UIElement):
    ...
    def w(self) -> int:
        ...
        for child in self.children:
            if hasattr(child, 'x'):
                child_w = getattr(child, 'w', 1)
                max_x = max(max_x, child.x + child_w - self.x)
        return max_x
    ...
    def h(self) -> int:
        ...
        for child in self.children:
            if hasattr(child, 'y'):
                child_h = getattr(child, 'h', 1)
                max_y = max(max_y, child.y + child_h - self.y)
        return max_y
    ...
    def cpp_struct_init(self) -> str:
        # MetaObjects are not directly represented in C++ structs in this design
        return ""

    def adjust_coords(self, dx: int, dy: int):
        self.x += dx
        self.y += dy
        for child in self.children:
            if hasattr(child, 'x'):
...
class GuiManager:
    ...
    def __init__(self, project_path: Path, update_interval: float = 0.2):
    ...
    def _lib_path(self) -> Path:
        return self._project_path.parent / "library.json"
    ...
    def edit_text_blocking(self, initial: str = "", title: str = "Edit Text") -> Optional[str]:
        ...
        # fallback if Tk not available
        if not self.ready.is_set() or not self._root:
            # fallback to single-line prompt (non-ideal)
            return initial
    ...
    def edit_props_blocking(self, obj: UIElement) -> Optional[Dict[str, Any]]:
        ...
        def open_dialog():
            ...
            if isinstance(obj, Box):
                ...
                txt.insert("1.0", "\n".join(obj.lines))
                row += 1
            elif isinstance(obj, MetaObject):
                tk.Label(frm, text="Children").grid(row=row, column=0, sticky="nw")
                children_list = tk.Listbox(frm, height=10)
                children_list.grid(row=row, column=1, sticky="ew", padx=4, pady=2)
                for child in obj.children:
                    children_list.insert(tk.END, f"{child.name} ({child.type})")
                row += 1
...
class ProjectManager:
    def __init__(self, project_path: Path):
        ...
        self.h_file = project_path.parent / "ui_layout.h"
        self.cpp_file = project_path.parent / "ui_layout.cpp"
        self.lib_file = project_path.parent / "SerialUI.h"

    def ensure_lib(self):
        try:
            if not self.lib_file.exists():
                self.lib_file.write_text(SERIAL_UI_HEADER, encoding="utf-8")
        except Exception:
            pass
    ...
    def save_project(self, screens: List[Screen]):
        ...
        def flatten_objects(objects: List[UIElement]) -> List[UIElement]:
            flat_list = []
            for obj in objects:
                if isinstance(obj, MetaObject):
                    flat_list.extend(flatten_objects(obj.children))
                else:
        ...
        try:
            ...
            for s in screens:
                ...
                for o in flatten_objects(s.objects):
                    cpp_type = f"UI_{o.type.capitalize()}"
                    h.append(f'    static const {cpp_type} {o.name};')
                h.append('};'); h.append(f'void drawScreen_{s.name}(SerialUI& ui);'); h.append('')
            h.append('#endif')
            self.h_file.write_text("\n".join(h), encoding="utf-8")
    ...
    def load_project(self) -> List[Screen]:
        try:
            if not self.project_path.exists(): return [Screen("Main")]
            txt = self.project_path.read_text(encoding="utf-8")
...
# ------------------------------
class Mode(Enum):
    NAV = auto(); BOX_1 = auto(); BOX_2 = auto(); LINE_1 = auto(); LINE_2 = auto(); MULTI_SELECT = auto(); RESIZE = auto()

class Designer:
    def __init__(self, stdscr, project_path: Path):
        self.stdscr = stdscr
        self.pm = ProjectManager(project_path)
        self.screens = self.pm.load_project()
        self.act_idx = 0
        self.mode = Mode.NAV
        self.cx = 5; self.cy = 5
        self.sel_idx = -1
        self.multi_select_indices: List[int] = []
        self.temp: Dict[str, int] = {}
        self.gui = GuiManager(project_path, update_interval=0.20)
    ...
    def _is_multi_selected(self, index: int) -> bool:
        return self.mode == Mode.MULTI_SELECT and index in self.multi_select_indices
    ...
    def run(self):
        ...
        for i, c in enumerate(Color, 1):
            try:
                # Use curses.COLOR_NAMES if available, otherwise fallback
                color_map = {
                    "RED": curses.COLOR_RED, "GREEN": curses.COLOR_GREEN, "YELLOW": curses.COLOR_YELLOW,
                    "BLUE": curses.COLOR_BLUE, "MAGENTA": curses.COLOR_MAGENTA, "CYAN": curses.COLOR_CYAN,
                    "WHITE": curses.COLOR_WHITE
                }
                color_name = c.name.split('_')[-1]
                curses.init_pair(i, color_map.get(color_name, curses.COLOR_WHITE), -1)
            except Exception:
                pass
    ...
    def _handle_key(self, k: int) -> bool:
        ...
        if dx or dy:
            if self.mode == Mode.NAV and self._valid_sel():
                ...
                if isinstance(o, Box):
                    ...
                    o.x2 += dx
                    o.y2 += dy
            elif self.mode == Mode.MULTI_SELECT:
                self.sel_idx = (self.sel_idx + dy) % len(self.cur_objs) if dy else self.sel_idx
                if dx > 0: self.sel_idx = min(len(self.cur_objs) - 1, self.sel_idx + 1)
                if dx < 0: self.sel_idx = max(0, self.sel_idx - 1)
            else:
        ...
        if self.mode == Mode.MULTI_SELECT:
            if k == ord(' '):
                if self.sel_idx in self.multi_select_indices:
                    self.multi_select_indices.remove(self.sel_idx)
                else:
                    self.multi_select_indices.append(self.sel_idx)
            elif k == ord('g'):
        ...
        if self.mode == Mode.NAV:
            ...
            if k == ord('m'):
                ...
                self.multi_select_indices = [self.sel_idx] if self._valid_sel() else []
                self.msg = "Multi-select mode. Use arrows, space to toggle, 'g' to group, 'u' to ungroup."
            if k == ord('g'):
                self._group_selection()
            if k == ord('u'):
                self._ungroup_selection()
            if k == ord('r') and self._valid_sel():
                ...
                self.msg = "Resize mode. Use arrows to resize. ESC to exit."
            if k == ord('b'):
        ...
        if self.mode == Mode.BOX_1 and k in (10, 13):
            ...
            new = Box(name=name, color=Color.WHITE, x=x, y=y, w=w, h=h)
            ...
            new = Line(name=name, color=Color.WHITE, x1=self.temp['x'], y1=self.temp['y'], x2=self.cx, y2=self.cy)
            self.cur_objs.append(new); self.sel_idx = len(self.cur_objs) - 1; self.mode = Mode.NAV; self.msg = f"Created {name}"
            try: self.gui.update_state(self.msg, [s.name for s in self.screens], self.cur_screen.name)
            except Exception: pass
    ...
    def _create_text(self):
        ...
        new = Text(name=name, color=Color.WHITE, x=self.cx, y=self.cy, content=str(txt))
    ...
    def _group_selection(self):
        if self.mode != Mode.MULTI_SELECT or len(self.multi_select_indices) < 2:
            self.msg = "Select at least two objects to group."
            return

        children = sorted([self.cur_objs[i] for i in self.multi_select_indices], key=lambda o: self.cur_objs.index(o))
        ...
        meta = MetaObject(name=f"meta_{len(self.cur_objs)}", x=int(min_x), y=int(min_y), children=children)

        # Remove original objects
        for i in sorted(self.multi_select_indices, reverse=True):
            self.cur_objs.pop(i)

        self.cur_objs.append(meta)
        self.sel_idx = len(self.cur_objs) - 1
        self.mode = Mode.NAV
        self.multi_select_indices = []
        self.msg = f"Grouped {len(children)} objects into {meta.name}."

    def _ungroup_selection(self):
        if not self._valid_sel() or not isinstance(self.cur_objs[self.sel_idx], MetaObject):
            self.msg = "Select a meta object to ungroup."
            return

        meta = self.cur_objs.pop(self.sel_idx)
        self.cur_objs.extend(meta.children)
        self.sel_idx = len(self.cur_objs) - len(meta.children)
        self.msg = f"Ungrouped {meta.name}."

    def _draw_element(self, element: UIElement, attr: int):
        try:
            if isinstance(element, Box):
                self.stdscr.addch(element.y, element.x, '+', attr)
                self.stdscr.addch(element.y + element.h - 1, element.x + element.w - 1, '+', attr)
                for k in range(1, max(1, element.w - 1)):
                    self.stdscr.addch(element.y, element.x + k, '-', attr)
                    self.stdscr.addch(element.y + element.h - 1, element.x + k, '-', attr)
                for k in range(1, max(1, element.h - 1)):
                    self.stdscr.addch(element.y + k, element.x, '|', attr)
                    self.stdscr.addch(element.y + k, element.x + element.w - 1, '|', attr)
            elif isinstance(element, Text):
                lines = element.content.splitlines() or [""]
                for r, ln in enumerate(lines):
                    self.stdscr.addstr(element.y + r, element.x, ln[:1000], attr)
            ...
            elif isinstance(element, Freehand):
                for r, ln in enumerate(element.lines):
                    self.stdscr.addstr(element.y + r, element.x, ln, attr)
            elif isinstance(element, MetaObject):
                # Draw a faint bounding box
                box_attr = curses.color_pair(list(Color).index(Color.WHITE) + 1) | curses.A_DIM
                try:
                    self.stdscr.addch(element.y, element.x, '+', box_attr)
                    self.stdscr.addch(element.y + element.h - 1, element.x + element.w - 1, '+', box_attr)
                except curses.error:
                    pass
                for child in element.children:
                    self._draw_element(child, attr)
        except curses.error:
            pass
    ...
    def _draw(self):
        ...
        for i, o in enumerate(self.cur_objs):
            ...
            if i == self.sel_idx or self._is_multi_selected(i):
                attr |= (curses.A_BOLD | curses.A_REVERSE)
            self._draw_element(o, attr)
...
# ------------------------------
def main():
    parser = argparse.ArgumentParser(description="A text-based UI designer for serial-connected devices.")
    parser.add_argument("--project", type=str, default="project.json", help="Path to the project JSON file.")
    parser.add_argument("--generate", action="store_true", help="Run in non-interactive mode to generate C++ files and exit.")
    ...
    if args.generate:
        try:
            ...
            print(f"Successfully generated C++ files for '{project_path}'.")
        except Exception as e:
            print(f"Error during generation: {e}")
    else:
        try:
            curses.wrapper(lambda scr: Designer(scr, project_path).run())
        except Exception as e:
            try: curses.nocbreak()
            except Exception: pass
            try: curses.echo()
