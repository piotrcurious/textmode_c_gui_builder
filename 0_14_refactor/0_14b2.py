...
// == COLORS ==
// Add 10 to make it bold in terminal emulators (e.g. FG_RED + 10)
#define FG_BLACK   30
#define FG_RED     31
#define FG_GREEN   32
#define FG_YELLOW  33
...
static inline void moveCursor(int16_t x, int16_t y) {
    Serial.print("\x1b["); Serial.print(y + 1); Serial.print(";"); Serial.print(x + 1); Serial.print("H");
}

static inline void setColor(uint8_t color) {
    if (color == 0) { Serial.print("\x1b[0m"); return; }
    if (color >= 40) {
        Serial.print("\x1b[1;"); Serial.print(color - 10);
    } else {
        Serial.print("\x1b[0;"); Serial.print(color);
    }
    Serial.print("m");
}
...
void draw(const Box& b) {
    ...
    // Top & Bottom
    for (int i = 0; i < b.w; i++) {
        moveCursor(b.x + i, b.y); Serial.print("-");
        moveCursor(b.x + i, b.y + b.h - 1); Serial.print("-");
    }
    // Left & Right
    for (int i = 0; i < b.h; i++) {
        moveCursor(b.x, b.y + i); Serial.print("|");
        moveCursor(b.x + b.w - 1, b.y + i); Serial.print("|");
    }
    // Corners
    moveCursor(b.x, b.y); Serial.print("+");
    moveCursor(b.x + b.w - 1, b.y); Serial.print("+");
    moveCursor(b.x, b.y + b.h - 1); Serial.print("+");
    moveCursor(b.x + b.w - 1, b.y + b.h - 1); Serial.print("+");
    resetAttr();
}

void draw(const Line& l) {
    setColor(l.color);
    int16_t x0 = l.x1, y0 = l.y1, x1 = l.x2, y1 = l.y2;
    int16_t dx = abs(x1 - x0), sx = x0 < x1 ? 1 : -1;
    int16_t dy = -abs(y1 - y0), sy = y0 < y1 ? 1 : -1;
    int16_t err = dx + dy, e2;
    
    while (true) {
        moveCursor(x0, y0); Serial.print("#");
...
void draw(const Freehand& f) {
    setColor(f.color);
    for (uint8_t i = 0; i < f.line_count; i++) {
        moveCursor(f.x, f.y + i);
        Serial.print(f.lines[i]);
    }
    resetAttr();
}
...
def cpp_declaration(name: str, obj: Drawable) -> str:
    match obj:
        case Box(x, y, w, h, color):
            return f"  Box {name} = {{{x}, {y}, {w}, {h}, {color.value}}};"
        case Text(x, y, content, color):
            return f'  Text {name} = {{{x}, {y}, "{cpp_escape(content)}", {color.value}}};'
        case Line(x1, y1, x2, y2, color):
            return f"  Line {name} = {{{x1}, {y1}, {x2}, {y2}, {color.value}}};"
        case Freehand(x, y, lines, color):
            arr = f"{name}_lines"
            esc = ", ".join([f'"{cpp_escape(l)}"' for l in lines])
            return (
                f"  const char* {arr}[] = {{{esc}}}; \n"
                f"  Freehand {name} = {{{x}, {y}, {arr}, {len(lines)}, {color.value}}};"
            )
    return ""

def cpp_draw_call(name: str) -> str:
    return f"  draw({name});"
...
HELP = {
    DesignMode.NAVIGATE: (
        "Draw: (b)ox (l)ine (t)ext (f)reehand | Move: arrows | "
        "Select: <TAB> | (c)olor (d)elete (+/-)layer | Save: (s) | Quit: (q)"
    ),
    DesignMode.DRAW_BOX_START: "Box: move to start point, ENTER to set, ESC to cancel",
    DesignMode.DRAW_BOX_END:   "Box: move to end point, ENTER to finish, ESC to cancel",
    DesignMode.DRAW_LINE_START: "Line: move to start point, ENTER to set, ESC to cancel",
    DesignMode.DRAW_LINE_END:   "Line: move to end point, ENTER to finish, ESC to cancel",
    DesignMode.GET_TEXT:       "Typing text... ENTER to accept",
...
class Designer:
    def __init__(self, stdscr):
        ...
        # objects: list of (name, Drawable)
        self.objects: list[tuple[str, Drawable]] = []
        self.selected: int = -1
        self.temp: dict[str, int] = {}
        self.status = HELP[self.mode]

    # --------------------- main loop ---------------------
    def run(self):
        curses.start_color()
        curses.use_default_colors()
        # Initialize pairs. -1 denotes default background.
    ...
    def _handle_key(self, key: int) -> bool:
        ...
        if self.mode in (DesignMode.DRAW_BOX_START, DesignMode.DRAW_BOX_END, 
                         DesignMode.DRAW_LINE_START, DesignMode.DRAW_LINE_END, 
                         DesignMode.NAVIGATE):
    ...
    def _handle_esc(self) -> bool:
        if self.mode in (DesignMode.DRAW_BOX_START, DesignMode.DRAW_BOX_END, 
                         DesignMode.DRAW_LINE_START, DesignMode.DRAW_LINE_END, 
                         DesignMode.GET_FREEHAND, DesignMode.GET_TEXT):
            self.mode = DesignMode.NAVIGATE
            self.temp.clear()
    ...
    def _prompt(self, prompt_text: str) -> str:
        ...
        edit_x = len(prompt_text) + 1
        edit_w = max(1, w - edit_x - 1)
        editwin = curses.newwin(1, edit_w, h - 1, edit_x)
        self.stdscr.refresh()
    ...
    def _ask_ident(self, base: str) -> str | None:
        ...
        if raw and not re_cpp_ident.match(raw):
    ...
    def _draw_preview(self) -> None:
        if self.mode == DesignMode.DRAW_BOX_END and {'x1','y1'} <= self.temp.keys():
            ...
            obj = Line(self.temp['x1'], self.temp['y1'], self.cursor_x, self.cursor_y)
            pair = 2
            try:
    ...
    def _generate_code(self) -> str:
        ...
        body = "\n".join(decls + ["", *draws])
        func = f"void drawScreen_main() {{\n{body}\n}}"
        
        code = CPP_TEMPLATE.replace("// [[SCREEN_FUNCTIONS_PLACEHOLDER]]", func)
        code = code.replace("// [[INITIAL_SCREEN_CALL_PLACEHOLDER]]", "drawScreen_main();")
        return code
...
if __name__ == "__main__":
    print("Starting Arduino Serial UI Designer (modernized, interactive)...")
    try:
        curses.wrapper(lambda stdscr: Designer(stdscr).run())
        print("Designer closed. If saved, 'ui_generated.ino' is ready.")
    except curses.error as e:
        print(f"Curses error: {e}")
        print("Your terminal may be too small or lacks full curses support.")
