...
class Designer:
    ...
    def _handle_input(self, key):
        if self.mode in [DesignMode.PROMPT, DesignMode.GET_TEXT, DesignMode.GET_FREEHAND, DesignMode.SWITCH_SCREEN]: return True
        ...
        if key == ord('q'): return False
            ...
            self.mode = DesignMode.NAVIGATE
            self.selected_object_index = -1
            self.status = "Edit/Draw cancelled."
        ...
        elif selected_obj: # Keys that apply only when an object is selected
            if key in [curses.KEY_DC, ord('d')]: self._delete_selected_object()
            elif key == ord('c'): self._change_selected_color()
            elif key == ord('+'): self._change_layer(1)
            elif key == ord('-'): self._change_layer(-1)
        else: # Keys that apply only when NO object is selected
            if key == ord('b'): self._start_drawing("box")
        ...
        # Enter key action depends on mode
        elif key in [10, 13, curses.KEY_ENTER]:
            if self.mode in [DesignMode.DRAW_BOX_START, DesignMode.DRAW_LINE_START]: self._set_first_corner()
            elif self.mode in [DesignMode.DRAW_BOX_END, DesignMode.DRAW_LINE_END]: self._finish_drawing()
        
        self._update_status()
        return True

    def _update_status(self):
        actions = "(n)ew (m)enu (s)ave | Draw: (b)ox (l)ine (t)ext (f)reehand | (TAB) select"
        if self._get_selected_object():
            actions = "MOVE with arrows | (d)elete (c)olor (+/-)layer (ESC) deselect"
        self.status = actions
    ...
    def _set_first_corner(self):
        self.temp_data['x1'], self.temp_data['y1'] = self.cursor_x, self.cursor_y
        if self.mode == DesignMode.DRAW_BOX_START: self.mode = DesignMode.DRAW_BOX_END
        elif self.mode == DesignMode.DRAW_LINE_START: self.mode = DesignMode.DRAW_LINE_END
